...
define('MFN_THEME_VERSION', '23.0.4');
...
if ( is_admin() ) {
	...
	// require_once(get_theme_file_path('/functions/admin/class-mfn-jess.php'));
	// require_once(get_theme_file_path('/functions/admin/class-mfn-jess-options.php'));
	// require_once(get_theme_file_path('/functions/admin/class-mfn-jess-builder.php'));
...
//Disable Accessibility Auto Update
add_filter( 'auto_update_plugin', 'disable_auto_update_for_specific_plugin', 10, 2 );

function disable_auto_update_for_specific_plugin( $update, $item ) {
    // Χρησιμοποίησε το σωστό slug
    if ( isset( $item->slug ) && $item->slug === 'pojo-accessibility' ) {
        return false;
    }
    return $update;
}

//Disable Accessibility Update IN GENERAL

add_filter( 'site_transient_update_plugins', 'hide_specific_plugin_update' );

function hide_specific_plugin_update( $value ) {
    if ( isset( $value->response['pojo-accessibility/pojo-accessibility.php'] ) ) {
        unset( $value->response['pojo-accessibility/pojo-accessibility.php'] ); // Αντικατάστησε με το plugin path που θέλεις να κρύψεις
    }
    return $value;
}
...
<p class="big">Μυκόνου 15 - Ζωγράφου, Τ.Κ. 15772</p>
