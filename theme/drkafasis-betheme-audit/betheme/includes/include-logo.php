...
echo '<div class="logo'. esc_attr($logo_class) .'">';
	...
	if (isset($logo_options['link'])) {
		$logo_before = '<a aria-label="Click on Logo to return to home page" id="logo" href="'. esc_url(get_home_url()) .'" title="'. esc_attr(get_bloginfo('name')) .'" data-height="'. intval($logo_height, 10) .'" data-padding="'. intval($logo_padding, 10) .'">';
		$logo_after = '</a>';
	} else {
	...
	if( mfn_opts_get( 'logo-img' ) || $logo_text || ( ! has_custom_logo() ) ){
		...
		if ( $logo_text ) {
			...
			foreach ( $logo['default'] as $logo_key => $logo_src ) {
				echo '<img class="logo-'. esc_attr( $logo_key ) .' scale-with-grid '. esc_attr( $svg ) .'" src="'. esc_url( $logo_src ) .'" data-retina="'. esc_url( $logo['retina'][$logo_key] ) .'" height="30px" data-height="'. esc_attr( mfn_get_attachment_data( $logo_src, 'height' ) ) .'" alt="'. esc_attr( mfn_get_attachment_data( $logo_src, 'alt' ) ) .'" width="'. esc_attr( $width ) .'" data-no-retina/>';
			}
